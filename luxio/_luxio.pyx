...
cdef class LuxIO:
    ...
    def __cinit__(self, object db_file_name, db_flags_t db_flag=DB_CREAT, index_type=NONCLUSTER, store_mode=Padded):
        ...
        if PyUnicode_Check(db_file_name):
            dbname = PyUnicode_AsUTF8String(db_file_name)
        else:
    ...
    def put(self, object key, object value, insert_mode=OVERWRITE):
        ...
        if PyUnicode_Check(value):
            valstr  = PyUnicode_AsUTF8String(value)
            vallen = valstr.length()
            valbuf = <char*>valstr.c_str()
        else:
    ...
    def get(self, object key):
        ...
        return PyBytes_FromStringAndSize(<const_char_ptr>value.data, <Py_ssize_t>value.size)
